...
Therefore, the production code needs to be built on Heroku. 

Prior to March, 2019, we used to have to update `package.json` to specify a `"heroku-postbuild"` script like this:
...
However, when deploying JS apps, Heroku will now automatically run a `"build"` script if it exists, which it does! So adding the `"heroku-postbuild"` script is no longer necessary.
